from project.engineer_astronaut import EngineerAstronaut
from project.scientist_astronaut import ScientistAstronaut
from project.research_station import ResearchStation
from project.maintenance_station import MaintenanceStation
from project.base_station import BaseStation
...
class SpaceAgency:
    ...
    def agency_update(self, min_value: float):
        ...
        for station in sorted_stations:
            astronaut_ids = " #".join(sorted(a.id_number for a in station.astronauts)) if station.astronauts else "N/A"
            total_salaries = sum(a.salary for a in station.astronauts)
            report.append(
                f"Station name: {station.name}; Astronauts: {astronaut_ids}; Total salaries: {total_salaries:.2f}")
